...
public class CsvBatchParser {
    public static CsvParseResult parseCsvFile(InputStream inputStream, String fileName, long fileSize) {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream))) {
            ...
            if (batch.getHeaderTotalRecords() == null || batch.getHeaderTotalAmount() == null) {
                return CsvParseResult.error("Archivo sin cabecera válida");
            }
            return CsvParseResult.success(batch, details);
        } catch (Exception e) {
            return CsvParseResult.error("Error al parsear CSV: " + e.getMessage());
        }
    }
    ...
    public static class CsvParseResult {
        ...
        public final PaymentBatch batch;
        public final List<PaymentDetail> details;

        private CsvParseResult(boolean success, String errorMessage, PaymentBatch batch, List<PaymentDetail> details) {
            this.success = success;
            this.errorMessage = errorMessage;
            this.batch = batch;
            this.details = details;
        }
        public static CsvParseResult success(PaymentBatch batch, List<PaymentDetail> details) {
            return new CsvParseResult(true, null, batch, details);
        }
        public static CsvParseResult error(String msg) {
            return new CsvParseResult(false, msg, null, null);
        }
    }
}